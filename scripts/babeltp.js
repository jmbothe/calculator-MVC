...
((function makeModel() {
  ...
  */
  function evaluateSubtotal() {
    if (this.operator.isLowPrecedence()) {
      this.subtotal.set(
        this.lowPrecedenceExpression.evaluate(
          this.midPrecedenceExpression.evaluate(
            this.highPrecedenceExpression.evaluate(this.input.get())
          )
        )
      );
  ...
  */
  ...
  function clearAfterEquals() {
    var lastButtonPressedWasEquals =
